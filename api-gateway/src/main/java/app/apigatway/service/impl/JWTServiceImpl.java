...
import app.apigatway.repository.TokenRepo;
...
public class JWTServiceImpl implements JWTService {
    ...
    @Override
    public Mono<String> getUserNameFromAccessToken(@NonNull String token) {
        return Mono.justOrEmpty(extractClaim(token,jwtAccessSecret,Claims::getSubject));
    }
    ...
    @Override
    public Mono<Boolean> isTokenValidAccessToken(@NonNull String token, @NonNull UserDetails userDetails){
        final Mono<String> username =getUserNameFromAccessToken(token);

      return   username.map(nameUser->
                nameUser.equals(userDetails.getUsername())&&!isTokenExpired(token,jwtAccessSecret)
                );

//        return Mono.just((username.flatMap(nameUser->nameUser.equals(userDetails.getUsername()))&&!isTokenExpired(token,jwtAccessSecret)));
    }
