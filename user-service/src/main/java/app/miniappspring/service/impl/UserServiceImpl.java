...
public class UserServiceImpl implements UserService {
    ...
    public CreateUserDto addUser(@NonNull CreateUserArgument createUserArgument) {
        ...
        if (isUsernameAlreadyInUse(createUserArgument.getUsername()))
            throw new RuntimeException("Пользователь с логином " + createUserArgument.getUsername() + " уже существует");
        ...
        User user = User.builder()
                ...
                .password(createUserArgument.getPassword())
    ...
    public UpdateDataUserDto updateDataUser(@NonNull UpdateDataUserArgument updateDataUserArgument) {
        ...
        User user = getByUsername(updateDataUserArgument.getUsername());
    ...
    public User getById(@NonNull Long id) {
        return userRepo.findById(id).orElseThrow(() -> new RuntimeException("Пользователь с id= " + id + " не найден"));
    }
    ...
    public User getByUsername(@NonNull String username) {
        try {
            return userRepo.findByUsername(username).orElseThrow(() -> new RuntimeException("Пользователь с логином " + username + " не найден"));
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public Set<Role> getListUserRole(String username) {
        return userRepo.findByUsername(username).orElseThrow(() -> new UsernameNotFoundException("Пользоватьль не найден")).getRoles();
    }
...
//    @Override
//    public UpdateDeliveryDataUser getDataUserAboutDelivery(String username) {
//
